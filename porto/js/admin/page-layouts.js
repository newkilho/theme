...
( function ( $ ) {
	if ( $( '.page-layouts' ).length ) {
		$( 'body' ).on( 'click', '.layout-box .layout-part', function () {
			var $this = $( this );
			if ( $this.hasClass( 'main-content' ) ) {
				return;
			}
			var $options = $( $( '#porto-layout-' + $this.attr( 'data-part' ) + '-options-html' ).html() ),
		...
		} ).on( 'click', '.layout-box .layout-header .back', function ( e ) {
			e.preventDefault();

			if ( !$( this ).closest( '.layout-box' ).hasClass( 'open-options' ) ) {
				return;
			}
			var $part_options = $( this ).closest( '.layout-box' ).find( '.part-options' );
			$part_options.find( '.builder-blocks' ).each( function () {
				$( this ).attr( 'data-value', $( this ).val() );
			} );
			$( '#porto-layout-' + $part_options.attr( 'data-part' ) + '-options-html' ).html( $part_options.html() );
			$part_options.attr( 'data-part', '' ).html( '' );
			$( this ).closest( '.layout-box' ).find( '.layout-header .status' ).remove();
			$( this ).closest( '.layout-box' ).removeClass( 'open-options' );

		} ).on( 'click', '.add-new-layout', function ( e ) {
			e.preventDefault();
			$( this ).prev().clone().insertBefore( $( this ).prev() ).removeClass( 'preset' );
		} ).on( 'click', '.layout-action-open', function ( e ) {
			e.preventDefault();
			var builder_id = $( this ).prev().prev().val();
			if ( builder_id == '' ) {
				alert( wp.i18n.__( 'Please select any block.' ) );
				return;
			}
			$.ajax( {
				url: ajaxurl,
				type: 'POST',
				data: {
					'action': 'porto_page_layouts_open_page',
					'builder_id': builder_id,
					'_nonce': porto_page_layouts.nonce
				},
				...
				failure: function () {
					alert( wp.i18n.__( 'Opening failed. Please refresh and try again.' ) );
				}
			} );
		} ).on( 'click', '.layout-action-remove', function ( e ) {
			e.preventDefault();
			var $this = $( this );
			var builder_id = $this.prev().prev().prev().val();
			$.ajax( {
				url: ajaxurl,
				type: 'POST',
				data: {
					'action': 'porto_page_layouts_remove_condition',
					'builder_id': builder_id == '' ? 0 : builder_id,
					'_nonce': porto_page_layouts.nonce,
					'data_part': $( '.page-layouts .part-options' ).attr( 'data-part' ) != '' ? $( '.page-layouts .part-options' ).attr( 'data-part' ) : 'top-block'
				},
				success: function ( response ) {
					$this.closest( '.option' ).fadeOut( 100, function () {
						$( this ).remove();
					} );
				},
				failure: function () {
					alert( wp.i18n.__( 'Removing failed. Please refresh and try again.', 'porto' ) );
				}
			} );
		} ).on( 'click', '.layout-action-condition', function ( e ) {
			e.preventDefault();
			var builder_id = $( this ).prev().val();
			if ( builder_id == '' ) {
				alert( wp.i18n.__( 'Please select any block.' ) );
				return;
			}
			$.ajax( {
				...
				type: 'POST',
				data: {
					'action': 'porto_page_layouts_display_condition',
					'builder_id': builder_id,
					'_nonce': porto_page_layouts.nonce
				},
				success: function ( response ) {
				...
				failure: function () {
					alert( wp.i18n.__( 'Loading failed. Please refresh and try again.' ) );
				}
			} );
			...
			check_duplicated_conditions = function ( $cond ) {
				...
				check_duplicated_conditions_timer = setTimeout( function () {
					...
					$.ajax( {
						...
						type: 'POST',
						dataType: 'json',
						data: { condition: new_condition, action: 'porto_page_layouts_check_condition', _nonce: porto_page_layouts.nonce, post_id: $cond.closest( 'form' ).find( '[name="post_id"]' ).val() },
						success: function ( response ) {
		...
		$( 'body' ).on( 'change', '.porto-builder-cond-wrap .condition-type', function ( e, is_first ) {
			var val = $( this ).val();
			$cond = $( this ).closest( '.porto-builder-condition' );
			if ( 'single' == val ) {
			...
			init_live_search( $new );
			check_duplicated_conditions( $new );
		} ).on( 'click', '.porto-builder-cond-wrap .save-condition', function ( e ) {
			e.preventDefault();
			var $this = $( this ),
				postdata = '';
			...
			$.ajax( {
				...
				type: 'POST',
				dataType: 'json',
				data: postdata + '&action=porto_page_layouts_save_condition&data_part=' + ( $( '.page-layouts .part-options' ).attr( 'data-part' ) != '' ? $( '.page-layouts .part-options' ).attr( 'data-part' ) : 'top-block' ),
				success: function ( response ) {
		...
		var init_live_search = function ( $obj ) {
			...
			$obj.each( function () {
				var $this = $( this ),
					...
					serviceUrl = ajaxurl + ( -1 === ajaxurl.indexOf( '?' ) ? '?' : '&' )  + 'action=porto_page_layouts_search_posts&_nonce=' + porto_page_layouts.nonce;
